...
class TestPlayerExitDeadState:
	...
	func test_dead_state_variables_unset_correctly() -> void:
		...
		assert_eq(_state_dead._parent.collision_mask, CollisionBit.WORLD + CollisionBit.PHYSICAL)
		assert_eq(_state_dead._parent.hurtbox.collision_mask, CollisionBit.PHYSICAL_DAMAGE)
