...
func generate_grid(grid: WorldGrid) -> void:
	...
	while(fails < GENERATOR_ATTEMPTS):
		...
		if (not lowered_chance_of_required_doors and 
			...
			push_warning("Room generation waiting on required_door_grid, lowering chance of required_doors being picked...")
		...
		if grid.number_of_rooms >= min_rooms:
			#gdlint:ignore=max-line-length
			print("Room generation waiting on constraints, lowering chance of non-constrained rooms being picked...")
			for entity_table_entry: EntityTableEntry in room_table.entities:
	...
	assert(false, "WorldGenerator failed it's singular task. You know who to blame!")
	return
